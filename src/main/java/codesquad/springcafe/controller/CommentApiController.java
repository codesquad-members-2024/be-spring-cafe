...
public class CommentApiController {
    ...
    @PostMapping
    public ResponseEntity<?> addComment(@PathVariable Long articleId, @Valid @RequestBody CommentWriteForm form,
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteComment(@PathVariable Long articleId, @PathVariable Long id,
                                           @SessionAttribute(LoginController.LOGIN_SESSION_NAME) LoginUser loginUser) {
        articleService.getArticle(articleId); // 아티클이 존재하는지 확인
