...
public class ArticleController {
    ...
    public String showArticle(Model model) {
        model.addAttribute("articles", MemoryArticleDatabase.getAllArticles()); // 전체 article 반환
    ...
    public String saveArticle(@ModelAttribute Article article, Model model) {
        article.setId(Integer.toString(MemoryArticleDatabase.getArticleSize()+1)); // article id 설정
        MemoryArticleDatabase.saveArticle(article);
    ...
    public String showArticle(@PathVariable Integer id, Model model) {
        if (MemoryArticleDatabase.isArticleEmpty()){
            return "redirect:/main";
        }

        Article article = MemoryArticleDatabase.getArticleById(id);
