...
## 필터와 인터셉터
- 모두 웹 어플리케이션에서 HTTP 요청과 응답을 처리하고 조작하는 데 사용되는 기능

### Filter
- Servlet의 생명주기 내에서 실행
- 클라이언트가 서블릿에 도달하기 전에 요청을 가로채고 응답을 가로채기 전에 후처리 가능
- 서블릿 컨테이너에 의해 직접 호출

### Interceptor
- 스프링 MVC의 핸들러 실행 전 또는 후에 실행
- 컨트롤러의 메서드가 실행되기 전 또는 후에 요청을 가로채고 응답을 가로채기 전에 후처리 가능
- Spring MVC의 DispatcherServlet에 의해 호출

### 적용 범위
- 적용되는 범위가 다르다
- 필터는 서블릿 컨테이너 레벨에서 동작하므로 모든 서블릿 및 리소스에 대해 적용
- 인터셉터는 스프링 MVC 내에서 동작하므로 스프링 MVC가 처리하는 컨트롤러에 대해서만 적용
- 일반적으로 필터는 보다 낮은 수준의 HTTP 요청 및 응답 처리에 사용, 인터셉터는 Spring MVC의 컨트롤러 및 핸들러 메서드와 관련된 더 높은 수준의 작업에서 사용

### 프로젝트에 적용하기
- 필터 기능을 활용해서 페이지 캐시 지우기 기능을 구현하고 싶다!
- 이전 페이지로 돌아간다고 해서 이전 페이지에서 작성했던 내용이 그대로 남아있거나 하지 않았으면 좋겠다

### 어떻게 구현하지?
- OncePerRequestFilter라는 Filter의 구현체를 활용


#### Filter 더 자세하게
- 필터 클래스는 sevlet의 Filter 인터페이스를 구현해 만들 수 있다
- init(), doFilter(), destroy() 메서드를 구현해야 한다
  - init() : 가장 먼저 필터를 인스턴스화 할 때 호출, 호출 전 뭔가 설정할 게 있다면 여기에서 코딩
  - doFilter() : ServletRequest, ServletResponse(Http~ 와 다르다), FilterChain 인자 세 개를 받는다. 필터의 기능을 구현한다
  - destroy() : 필터 인스턴스 제거하는 곳. 삭제되기 전 뭔가 청소 작업을 해야 한다면 여기에서 코딩
- FilterChain?
  - 필터는 모듈로 블록처럼 쌓을 수 있도록 고안된 것이기에 다양한 방식으로 섞어서 사용할 수 있는데, 이를 가능하게 해주는 가장 중요한 부품
  - 다음에 실행할 필터가 어떤 것이다라는 것을 알고 있다
- chain.doFilter()?
  - FilterChain의 doFilter()와 Filter의 doFilter()는 다르다
  - FilterChain의 doFilter()는 다음 호출할 필터가 뭔지 파악해 그 필터의 doFilter()를 실행한다
  - 물론 체인의 마지막이라면 서블릿의 service() 메서드 호출.

#### 지금까지는 요청 필터... 응답 필터는?
- 서블릿이 작업을 완료한 다음, 응답을 클라이언트로 보내기 전에 응답 출력에 어떤 작업을 할 수 있다
- 서블릿은 자기 할 일을 한 뒤 출력을 만든다. 출력에 대한 제어는 컨테이너로 넘어가고 클라이언트로 넘어간다. 이미 클라이언트로 넘어간 출력을 필터가 제어할 수 없다...
- 서블릿에 실제 response 객체를 넘기지 말고, 필터에서 만든 response 객체로 바꿔치기 하자!
  - 컨테이너는 자신이 쓸 요량으로 HttpServletResponse 인터페이스 구현 객체를 만들어 두었는데, 필터에서 이 구현 객체를 만들어 chain.doFilter()에 넘겨야. 출력 스트림도 포함
  - 새로 만든 풀력 스트림에 서블릿이 기록을 하면 클라이언트로 넘기기 전에 필터가 제어할 수 있다
- 근데 언제 다 구현하지?
  - 랩퍼를 사용하자

#### 랩퍼
- 원본 request, response 객체를 내부에 감싸고, 자신에게 들어오는 모든 메서드를 바로 이 객체로 위임하는 식으로 메서드를 구현
- 개발자는 랩퍼 클래스를 상속해 필요한 메서드만 재정의하면 된다
- 랩퍼 클래스는 현재 구현하려는 타입의 객체를 내부에 가지고 있다(감싸고 있다)
- 인터페이스를 직접 구현하는 것이 아니라, 모든 메서드 호출을 위임할 동일한 타입의 클래스 객체에 대한 참조를 내부에 가지고 있다
- 종류
  - SevletRequestWrapper
  - HttpServletRequestWrapper
  - ServletResponseWrapper
  - HttpServletRequestWrapper
- 여기까지만 알아보고, 자세한 건 나중에 필요하면 더!

#### doFilter과 doFilterInternal
- 둘 다 클래스에서 제공되는 메서드
- Filter 인터페이스 구현, 이 메서드를 오버라이드해 필터링 로직 구현
- doFilter는 Servlet 컨테이너에 의해 필터 체인을 통해 호출, doFilterInternal은 스프링 프레임워크에 의해 필터 체인을 통해 호출
- doFilter는 FilterChain을 매개변수로 받아 다음 필터로 요청을 전달하거나, 필터 체인의 끝에 도달하면 실제로 요청을 처리
- doFilterInternal은 FilterChain과 함께 HttpServletRequest 및 HttpServletResponse를 매개변수로 받아 처리
  - 실제로 요청을 처리하거나 다음 필터로 전달하는 것은 해당 클래스가 처리
---
## 참고 링크
