...
- 해결:
  - '테이블명_칼럼명'형태로 alias를 만드는 메서드를 만들어 해당 alias로 데이터를 조회해 매핑했다.
  - SimpleRowMapper의 mapFields 메서드에서는 ResultSet에서 getObject를 할 때 테이블명_칼럼명을 읽어 값을 가져와 할당한다
  - 또한 users와 같은 테이블의 경우 클래스명으로 조회하면 오류가 발생하므로 Table이라는 어노테이션을 만들어 name에 작성된 이름으로 테이블명을 사용했다
  - 이 과정을 통해 연관관계 매핑 시 가장 하고 싶었던 필드로 아이디 가지지 않고 객체 자체 가지도록 하기도 성공했다.
...
### 타임리프 문법 @와 $의 차이
- 문제:
  - 회원탈퇴 시 취소 버튼을 누르면 뒤로 가기가 되어야 하는데, 잘못된 값이 전달되어 다른 페이지로 이동하고 있었다
- 해결:
  - `th:href=@{goBack}`으로 값을 주고 있었는데, `@{${goBack}}`으로 바꾸니 해결됐다.
  - @{...} 표현식
    - url 표현식으로 타임리프에서 url을 생성하는 데 사용된다
    - 현재 컨텍스트 경로에 표현식 안의 값을 추가해 url을 생성한다
